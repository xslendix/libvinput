...
void keyboard_callback(KeyboardEvent evt) {
  ...
  if (!isprint(evt.keychar) && !isspace(evt.keychar))
    return;

  if (!isalpha(evt.keychar) ||
      VInput_modifier_pressed_except_shift(evt.modifiers)) { // New word
    if (word_len)
      printf("Word introduced: %.*s\n", word_len, word_buffer);
    word_len = 0;
  } else {
    if (word_len > sizeof(word_buffer) / sizeof(char))
      word_len = 0;
