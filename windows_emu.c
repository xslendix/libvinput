...
VInputError Emulator_keyboard_state_get(Emulator *emulator, int **state,
                                        int *nstate) {
	(void)nstate;

  if (!emulator->initialized)
    return VINPUT_UNINITIALIZED;

  BYTE states[256];
  if (!GetKeyboardState((PBYTE)states))
    return VINPUT_WINAPI_GETKEYBOARDSTATE;
...
VInputError Emulator_keyboard_state_clear(Emulator *emulator) {
  ...
  while (*cmod) {
    ...
    if (SendInput(1, &input, sizeof(INPUT)) != 1)
      return VINPUT_WINAPI_SENDINPUT;
  }

  return VINPUT_OK;
}

VInputError Emulator_keyboard_state_set(Emulator *emulator, int *state,
                                        int nstate) {
	(void)nstate;
